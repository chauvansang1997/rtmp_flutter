...
internal class MethodCallHandlerImpl(
    ...
    private val methodChannel: MethodChannel = MethodChannel(messenger, "plugins.flutter.io/camera_with_rtmp")
    private val imageStreamChannel: EventChannel = EventChannel(messenger, "plugins.flutter.io/camera_with_rtmp/imageStream")
    private var camera: Camera? = null
    private var rtmpManager: RtmpManager? = null
...
//            }
            "startVideoStreaming" -> {
                ...
                rtmpManager!!.startConnection(call.argument("url"),
                        bitrate,
                        result)
...
//                        result)
            ...
            "setImageFilter" ->{
               val  byteArray : ByteArray? = call.argument("image")
                rtmpManager!!.setImageFilter(data = byteArray, result = result)
            }
    ...
    private fun instantiateCamera(call: MethodCall, result: MethodChannel.Result) {
        ...
        camera = Camera(
                ...
                enableOpenGL)
        camera!!.open(result)
        rtmpManager = camera?.let { RtmpManager(camera = it, activity = activity, dartMessenger = dartMessenger) }
    }
